...
void output_levelset(levelset_vec *G, parameter *para) {
  ...
  sprintf(buffer, "./data/maxz_%.1f_maxr_%.1f_dz_%.2f_vf_%.2f_tlow_%.1f_thigh_%.1f_twidth_%.1f_lowtwidth_%.1f/outputG_t_%.6f", para->maxz, para->maxr, para->dz, para->vf, para->tlow, para->thigh, para->twidth, para->lowtwidth, G->t);
...
void output_uwp(stokes_force *uwp, parameter *para){
  ...
  sprintf(buffer, "./data/maxz_%.1f_maxr_%.1f_dz_%.2f_vf_%.2f_tlow_%.1f_thigh_%.1f_twidth_%.1f_lowtwidth_%.1f/outputUWP_t_%.6f", para->maxz, para->maxr, para->dz, para->vf, para->tlow, para->thigh, para->twidth, para->lowtwidth, uwp->t);
